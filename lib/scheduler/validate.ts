import { addDaysToDateStr, eachDateInRange, getKstParts, isoToEpoch, parseHm } from "@/lib/time";
...
function endMinuteOnStartDate(
  startParts: ReturnType<typeof getKstParts>,
  endParts: ReturnType<typeof getKstParts>,
): number | null {
  const startDate = dateStrOf(startParts);
  const endDate = dateStrOf(endParts);
  if (startDate === endDate) return endParts.hours * 60 + endParts.minutes;
  const isNextMidnight =
    endDate === addDaysToDateStr(startDate, 1) && endParts.hours === 0 && endParts.minutes === 0;
  return isNextMidnight ? 24 * 60 : null;
}

/**
 * (#1) 확정하려는 슬롯이 유효한지 서버에서 직접 판정한다.
 * 후보 "목록(top-N)" 포함 여부가 아니라, 후보 생성/제외 규칙을 그대로 적용한다.
 * - 진행 시간 일치, 30분 시작 정렬, 날짜 범위와 선택 가능 시간 안
 * - 점심 시간과 비겹침
...
export function isSlotConfirmable(
  ...
  if (!eachDateInRange(meeting.dateStart, meeting.dateEnd).includes(dateStrOf(sp))) {
    return { ok: false, reason: "일정 후보 기간 밖의 시간이에요." };
  }
  ...
  if (startMin < workStart || endMin > workEnd) {
    return { ok: false, reason: "선택 가능한 시간 범위 밖이에요." };
  }

  const lunchStart = parseHm(meeting.lunchStart);
  const lunchEnd = parseHm(meeting.lunchEnd);
  if (
    isBreakWindowEnabled(meeting.lunchStart, meeting.lunchEnd) &&
    overlaps(startMin, endMin, lunchStart, lunchEnd)
  ) {
  ...
  if (requiredBusy) {
    return { ok: false, reason: "꼭 함께할 사람이 참여할 수 없는 시간이에요." };
  }
...
export function validateSubmittedBlocks(
  ...
  for (const b of blocks) {
    ...
    if (!dates.has(dateStrOf(sp))) {
      return { ok: false, reason: "일정 후보 기간 밖의 응답이 있어요." };
    }
    ...
    if (startMin < workStart || endMin > workEnd) {
      return { ok: false, reason: "선택 가능한 시간 범위 밖의 응답이 있어요." };
    }
    if (
      isBreakWindowEnabled(meeting.lunchStart, meeting.lunchEnd) &&
      overlaps(startMin, endMin, lunchStart, lunchEnd)
