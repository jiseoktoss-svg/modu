...
- 회의 생성: 한 항목씩 입력하면 상단 안내 문장이 완성되는 "문장 빌더" 흐름(회의명 → 안건 → 장소 → 회의 마감 날짜 → 예상 회의 진행 시간(시간/분) → 참석자 순). 모든 항목 필수이며, 상단 문장의 값을 클릭하면 해당 항목으로 돌아가 수정. 회의 마감 날짜는 요일을 표시하는 커스텀 캘린더(`components/ui/DatePicker.tsx`, KST 안전, 최소 오늘)로 선택. 참석자는 가상 직원 50명 검색·클릭으로 추가하고 필수참석/선택참석 두 구역에서 드래그 앤 드롭으로 분류(빈 곳 드래그 시 사각형 범위 선택으로 여러 명 동시 이동, 데스크톱 기준)하며 최소 2명 필요(1명 이하로 `선택 완료` 시 모달 내 유효성 경고). 생성 시 참석자별 토큰 발급, 참석자 링크 생성
...
Next.js 14 (App Router) · TypeScript · Tailwind CSS · Supabase(Postgres) · Vitest · 토스 디자인 시스템(`@toss/tds-mobile`, `@toss/tds-mobile-ait`, `@apps-in-toss/web-framework`, `@emotion/react`)

UI는 기본적으로 TDS Mobile 컴포넌트를 따릅니다: 버튼은 `TDSButton`(TDS `Button`/`CTAButton`/`BottomCTA` 래퍼), 숫자 입력은 `NumericSpinner`, 검색은 `SearchField`, 아이콘 버튼은 `IconButton`을 쓰고, 앱은 `TDSMobileAITProvider`(`components/providers/AppProviders.tsx`)로 감쌉니다. (Apps in Toss 미니앱 톤을 지향하되 일반 웹으로도 동작합니다.)

본문 텍스트는 Pretendard(`globals.css` 에서 CDN `@import`, Tailwind `fontFamily.sans` 에도 반영)로 렌더링합니다. 이모지는 `TossFace` 폰트로 렌더링하며(`globals.css` 의 `@font-face` `unicode-range` 로 이모지 코드포인트에만 적용), `public/fonts/TossFaceFontWeb.otf`(COLR/CPAL, 전 브라우저 컬러)를 우선 사용하고 `TossFaceFontMac.ttf` 로 폴백합니다. UI 아이콘은 lucide-react 대신 유니코드 이모지를 `components/ui/Emoji.tsx`(TossFace) 또는 TDS `IconButton`으로 렌더링합니다.
