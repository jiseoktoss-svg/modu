...
- 참석자 응답은 문장 빌더 흐름: 링크 진입 시 회의 안내(intro) → `시간 정하러 가기` → 본인 확인(이름 입력(라벨 옆 `?` 아이콘으로 본인확인 이유 툴팁) → 직무 선택, 명단 검증, 오류는 토스트, 성공 시 확인 완료 토스트) → **불가 중심 가능 시간 2단계**(① 불가능한 날짜 `busy` ② 특정 날짜+시간 불가 `busy`, 둘 다 다중 입력, 월 이동(`<`/`>`) 달력·토일 비활성) → 입력 확인(review, `응답 시간 마감 전까지 수정할 수 있어요` 안내) → 제출. 날짜·시간 선택 모달은 **모바일은 전체화면 시트, 데스크톱은 가운데 카드**로 뜬다(모바일 ②는 한 시트에서 날짜+시간을 함께 고르고, 데스크톱은 날짜 모달 후 페이지에서 시간 입력). 근무시간 밖 시간은 컨펌 후 허용, 점심시간과 겹치는 블록은 제외. 같은 브라우저 수정(localStorage 토큰)이며, 복귀 사용자는 대기 화면(waiting)으로 진입
- 제출 후 흐름: 입력 확인(review) → **대기(waiting)** → **후보 화면(result)** → `캘린더 보기` 로 **캘린더(done)** 이동. 대기 화면은 세로 스텝퍼 + 카운트다운(데모용 5초)이며, 마감되면 마지막 스텝(`추천시간을 확인해보세요`)이 활성화된다. 후보·캘린더 화면은 현재 `docs/cases.md` 의 8개 케이스를 보여주는 데모(`data/demoCases.ts`)로 동작하며, 상단의 케이스 선택 버튼(1~8, 호버 시 펼침)으로 상황·솔루션·경고 배너를 전환한다. 실제 회의 생성/응답과는 무관
- 후보 화면: 순위·시간·등급 배지·득표·이유가 담긴 카드 목록. 페이지는 정상 스크롤되고 하단 `캘린더 보기` CTA만 `MobileStickyAction`으로 고정한다(응답 수정은 제공하지 않음 — 수정은 대기 화면에서). 카드 호버 시 `투표하기` → 누르면 `투표됨` 배지가 한 번에 한 후보에만 붙는다. `buildCaseCandidates` 가 §8.5 우선순위(필수 전원 가능 → 필수 많이 → 선택 많이 → 충돌 적음 → 이른 시간)로 정렬하고 등급을 도출
