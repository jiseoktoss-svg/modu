...
- 회의 생성(회의명/안건/장소/회의 마감 날짜/회의 길이 직접 입력/가상 직원 50명 검색 선택/필수 참석자 분류) + 참석자별 토큰 발급, 참석자 링크 생성
- 로그인 없는 본인 확인(이름/직무 명단 검증) 후 날짜 선택 + 날짜별 30분 단위 시간 응답, 같은 브라우저 수정(localStorage 토큰)
...
- 확정 후 공유 문구 + `.ics` 다운로드
- 샘플 회의(6명, 미응답·외근·점심직후 포함) 원클릭 체험
...
이모지 렌더링은 `public/fonts/TossFaceFontMac.ttf`를 `TossFace` 폰트로 등록해 사용합니다.
