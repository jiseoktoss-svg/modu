...
// 회의 만들기(components/ui/DatePicker.tsx)의 달력 모달과 시각적으로 동일한 톤을 재사용하되,
// (a) 다중 선택, (b) 모바일 전체화면(상/하 테두리 제거·좌우 꽉참·선택 날짜 칩)까지 지원하는
// 응답 폼 전용 공용 달력 모달. DatePicker 를 import 하지 않고 마크업을 복제해 "동일화"한다.
        ...
        {grid.map((d, i) => {
          ...
          return (
            <button
              ...
              onClick={() => onToggle(ds)}
              aria-pressed={isSel}
              aria-label={blocked ? `${m}월 ${d}일 — 불가능한 날짜` : `${m}월 ${d}일${isSel ? " 선택됨" : ""}`}
              className={cn(
